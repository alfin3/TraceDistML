...
Higher order functions enable propagation of changes in OpenTracing span granularity. 
TODO: segmentation of gradient ops.
...
class CHAR_LSTM(object):
    ...
    def _create_placeholders(self):
        ...
        with tf.name_scope('data'), self.optr_tracer.start_active_span('data'):
            ...
            with tf.name_scope('lstm'), self.optr_tracer.start_active_span('lstm'):
                ...
                with tf.name_scope('vars'), self.optr_tracer.start_active_span('vars'): 
                    lstm = lstm_fn(topology=self.lstm_topology,
                                   optr_tracer=self.optr_tracer)
                with tf.name_scope('ops'), self.optr_tracer.start_active_span('ops'):
                    (self.train_outputs, 
                     self.val_output, 
            ...
            with tf.name_scope('classifier'), self.optr_tracer.start_active_span('classifier'):
                with tf.name_scope('vars'), self.optr_tracer.start_active_span('vars'):
                    classifier = classifier_fn(topology=self.classifier_topology, 
                                               ...
                                               optr_tracer=self.optr_tracer)
                with tf.name_scope('train_ops'), self.optr_tracer.start_active_span('train_ops'):
                ...
                with tf.name_scope('val_ops'), self.optr_tracer.start_active_span('val_ops'): 
                    with tf.control_dependencies(
                        [saved_output.assign(to_save_output) for saved_output, to_save_output in 
    ...
    def _create_training(self):
        ...
        with tf.device(self.training_device):
            ...
            with tf.name_scope('gradient_descent'), \
                 self.optr_tracer.start_active_span('gradient_descent'):
                self.optimizer = tf.train.GradientDescentOptimizer(self.learning_rate)
                gradients = []
                v = []
                with self.optr_tracer.start_active_span('gradients'):
                    gradients, v = zip(* self.optimizer.compute_gradients(self.loss))
                with self.optr_tracer.start_active_span('clip'):
                    gradients, _ = tf.clip_by_global_norm(gradients, self.clip_norm)
                with self.optr_tracer.start_active_span('apply_gradients'):
    ...
    def build_graph(self):
        """Build the graph."""
        with self.optr_tracer.start_active_span('build_graph'):
...
def cell_fn(in_dim, out_dim, optr_tracer):
    """Build a cell function."""
    with tf.name_scope('input_vars'), optr_tracer.start_active_span('input_vars'):
        input_weights, input_biases = cell_variables(in_dim, out_dim)
    with tf.name_scope('forget_vars'), optr_tracer.start_active_span('forget_vars'):
        forget_weights, forget_biases = cell_variables(in_dim, out_dim)
    with tf.name_scope('output_vars'), optr_tracer.start_active_span('output_vars'):
        output_weights, output_biases = cell_variables(in_dim, out_dim)
    with tf.name_scope('update_vars'), optr_tracer.start_active_span('update_vars'):
    ...
    def cell_ops(input_data, output_data, state):
        ...
        with tf.name_scope('input_ops'), optr_tracer.start_active_span('input_ops'):
            input_gate = tf.sigmoid(tf.matmul(data, input_weights) + input_biases)
        with tf.name_scope('forget_ops'), optr_tracer.start_active_span('forget_ops'):
            forget_gate = tf.sigmoid(tf.matmul(data, forget_weights) + forget_biases)
        with tf.name_scope('output_ops'), optr_tracer.start_active_span('output_ops'):
            output_gate = tf.sigmoid(tf.matmul(data, output_weights) + output_biases)
        with tf.name_scope('update_ops'), optr_tracer.start_active_span('update_ops'):
            update = tf.matmul(data, update_weights) + update_biases
        with tf.name_scope('state_ops'), optr_tracer.start_active_span('state_ops'):
...
def lstm_layer_fn(in_dim, out_dim, layer_name, optr_tracer):
    """Build a function for an LSTM layer."""
    with tf.name_scope(layer_name + '_vars'), \
         optr_tracer.start_active_span(layer_name + '_vars'):
        cell = cell_fn(in_dim=in_dim + out_dim, 
                       out_dim=out_dim, 
                       optr_tracer=optr_tracer)  
...
def lstm_fn(topology, optr_tracer):
...
def classifier_layer_fn(in_dim, out_dim, layer_name, optr_tracer, fn=tf.nn.relu):
...
def classifier_fn(topology, keep_prob, optr_tracer):
